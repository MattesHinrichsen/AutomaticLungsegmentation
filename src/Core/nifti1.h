...
struct nifti_1_header { /* NIFTI-1 usage         */  /* ANALYZE 7.5 field(s) */
 ...
 char magic[4] ;      /*!< MUST be "ni1\0" or "n+1\0". */
  ...
  operator nifti_2_header() {
    nifti_2_header hdr2;

    hdr2.sizeof_hdr = 540;

    std::copy(magic, magic+4, hdr2.magic);         /*  Changes with .hdr and .nii
    
    Nifti1 .nii: {0x6E, 0x2B, 0x31, 0x00};
    Nifti1 .hdr: {0x6E, 0x69, 0x31, 0x00};
    Nifti2 .nii: {0x6E, 0x2B, 0x32, 0x00, 0x0D, 0x0A, 0x1A, 0x0A}
    Nifti2 .hdr: {0x6E, 0x69, 0x32, 0x00, 0x0D, 0x0A, 0x1A, 0x0A} 
    */

    hdr2.datatype = datatype;
    hdr2.bitpix = bitpix;
    std::copy(dim, (dim + 8), hdr2.dim);           //dim array copy
    hdr2.intent_p1 = intent_p1;
    hdr2.intent_p2 = intent_p2;
    hdr2.intent_p3 = intent_p3;
    std::copy(pixdim, pixdim+8, hdr2.pixdim);
    hdr2.vox_offset = vox_offset;                  //Changes with .hdr and .nii probalbly 544
    hdr2.scl_slope = scl_slope;
    hdr2.scl_inter = scl_inter;
    hdr2.cal_max = cal_max;
    hdr2.cal_min = cal_min;
    hdr2.slice_duration = slice_duration;
    hdr2.toffset = toffset;
    hdr2.slice_start = slice_start;
    hdr2.slice_end = slice_end;
    std::copy(descrip, descrip+80, hdr2.descrip);
    std::copy(aux_file, aux_file+24, hdr2.aux_file);
    hdr2.qform_code = qform_code;
    hdr2.sform_code = sform_code;
    hdr2.quatern_b = quatern_b;
    hdr2.quatern_c = quatern_c;
    hdr2.quatern_d = quatern_d;
    hdr2.qoffset_x = qoffset_x;
    hdr2.qoffset_y = qoffset_y;
    hdr2.qoffset_z = qoffset_z;
    std::copy(srow_x, srow_x+4, hdr2.srow_x);
    std::copy(srow_y, srow_y+4, hdr2.srow_y);
    std::copy(srow_z, srow_z+4, hdr2.srow_z);
    hdr2.slice_code = slice_code;
    hdr2.xyzt_units = xyzt_units;
    hdr2.intent_code = intent_code;
    std::copy(intent_name, intent_name+16, hdr2.intent_name);
    hdr2.dim_info = dim_info;
    // hdr2.unused_str only in Nifti2 
